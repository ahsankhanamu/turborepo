...
function persistWidgetStore() {
  chrome.storage.local.set({ widgetStore }, () => {
    console.log('Widget store persisted.');
  });
}

// Restore the widget store from Chrome's local storage
...
// Restore the widget store when the script is loaded
restoreWidgetStore();
