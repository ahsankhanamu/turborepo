...
const JsonViewer = ({ data }) => {
  // Flatten structure at the top level and render TreeNode for each item
          ...
          <tbody>
            {Object.keys(data).map((key, index) => (
              <React.Fragment key={index}>
                ...
                />
                {isExpanded(key) && typeof data[key] === 'object' && data[key] !== null && (
                  <TreeNodeChildren
                    parentKey={key}
                    value={data[key]}
                    level={1}
...
const TreeNode = ({ nodeKey, value, level, expanded, onToggle }) => {
  ...
  return (
    <tr>
      ...
      <td>
        <button className="contextmenu" title="Click to open the actions menu (Ctrl+M)"></button>
      </td>
      <td>
        <table className="values border-collapse" style={{ marginLeft: `${level * 24}px` }}>
          <tbody>
            <tr>
              <td className="tree">{isObject && <button className="invisible"></button>}</td>
              <td className="tree">
