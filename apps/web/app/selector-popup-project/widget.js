...
(function () {
  ...
  function addSingleElementToSelection(element) {
    if (toolManager.currentTool !== 'singleSelect' || (popup && popup.contains(element))) return;
    if (popupUtils.isValidElement(element)) {
      ...
      // popupUtils.attachInspectPopup(element); // Integrate popupUtils for the single select tool
      createPopup();
      updatePopup();
    }
  }
